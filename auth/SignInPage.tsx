...
const SignInPage: React.FC<SignInPageProps> = ({ onSignInSuccess }) => {
  ...
  const handleSubmit = async (event: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      // onAuthStateChange in App.tsx will handle setting isAuthenticated and navigation
      // For now, we can directly call onSignInSuccess if needed, but App.tsx should be the primary handler
      onSignInSuccess(); 
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-gray-900 to-black text-white flex flex-col items-center justify-center p-4">
      ...
      >
        <h1 className="text-4xl font-bold mb-4 text-amber-400">Sign In</h1>
        <p className="text-gray-300 mb-8">Welcome back! Sign in to your Integral account.</p>
         ...
         <form className="space-y-6" onSubmit={handleSubmit}>
          <div>
            ...
            <input 
              ...
              name="email" 
              id="email" 
              className="w-full px-4 py-2.5 rounded-md bg-gray-700 border border-gray-600 text-white focus:ring-amber-500 focus:border-amber-500 placeholder-gray-500" 
          ...
          <Button 
            type="submit" 
            size="lg" 
            className="w-full bg-gradient-to-r from-amber-500 to-amber-700 hover:from-amber-600 hover:to-amber-800 text-white shadow-lg"
        ...
        <p className="mt-8 text-gray-400">
          ...
          <Link to="/signup" className="font-medium text-amber-400 hover:text-amber-500">
            Sign Up
          </Link>
        </p>
      </motion.div>
